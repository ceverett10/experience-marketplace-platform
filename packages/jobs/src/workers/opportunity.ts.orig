...
export async function handleOpportunityScan(
  ...
  try {
    ...
    const holibobClient = createHolibobClient({
      ...
      sandbox: process.env["HOLIBOB_ENV"] === "sandbox",
    });
...
> {
  ...
  for (const destination of targetDestinations) {
    for (const category of targetCategories) {
      const keyword = `${destination?.split(',')[0]?.toLowerCase()} ${category}`;
      ...
      try {
        ...
        const inventory = await holibobClient.productDiscovery({
          where: { freeText: destination },
          what: { data: { searchTerm: category } },
        });

        const inventoryCount = inventory.recommendedProductList?.nodes?.length || 0;
        ...
        console.error(`[Opportunity Scan] Error checking inventory for ${keyword}:`, error);
      }
    }
  }
