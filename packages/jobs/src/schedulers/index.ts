...
export async function initializeScheduledJobs(): Promise<void> {
  ...
  // createBooking) twice daily. Catches API contract breaks before users hit
  ...
  await scheduleJob('AD_SEARCH_TERM_HARVEST' as any, {}, '0 4 * * 2,4');
  console.info('[Scheduler] ✓ Ad Search Term Harvest - Tuesdays & Thursdays at 4 AM');
...
export async function removeAllScheduledJobs(): Promise<void> {
  ...
  await queueRegistry.removeRepeatableJob('AD_SEARCH_TERM_HARVEST' as any, '0 4 * * 2,4');
