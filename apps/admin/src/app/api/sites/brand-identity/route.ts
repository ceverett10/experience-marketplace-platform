...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const { siteIds, regenerateAll = false, themeGuidance } = body;
    ...
    for (const site of sites) {
      try {
        ...
        // Serialize themeGuidance into generationPrompt for recall on next regeneration
        const generationPrompt = themeGuidance
          ? JSON.stringify({ themeGuidance, niche: context.niche, keyword: context.keyword })
          : `Brand identity for ${context.niche} in ${context.keyword}`;
        ...
        // Store themeGuidance in seoConfig so downstream content generation can reference it
        if (themeGuidance) {
          const existingSeoConfig = (site.seoConfig as Record<string, unknown>) || {};
          await prisma.site.update({
            where: { id: site.id },
            data: {
              seoConfig: { ...existingSeoConfig, themeGuidance },
            },
          });
        }
