...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const { generateComprehensiveBrandIdentity, storeBrandIdentity, generateLogo, isLogoGenerationAvailable } =
    ...
    for (const site of sites) {
      try {
        ...
        // Generate logo if DALL-E + R2 configured
        let logoUrl: string | null = site.brand?.logoUrl || null;
        if (isLogoGenerationAvailable()) {
          try {
            console.log(`[Brand Identity] Generating logo for ${site.name}...`);
            ...
            console.log(`[Brand Identity] Logo generated: ${logoUrl}`);
          } catch (logoErr) {
