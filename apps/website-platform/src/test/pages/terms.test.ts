...
describe('Terms page generateMetadata', () => {
  ...
  it('sets canonical URL', async () => {
    const { generateMetadata } = await import('@/app/terms/page');
    const meta = await generateMetadata();
    expect(meta.alternates?.canonical).toBe('https://test.example.com/terms');
  });

  it('includes openGraph with site name suffix', async () => {
    const { generateMetadata } = await import('@/app/terms/page');
    const meta = await generateMetadata();
    expect(meta.openGraph?.title).toBe('Terms of Service | Test Site');
  });
  ...
  it('uses primaryDomain in canonical', async () => {
    ...
    const { generateMetadata } = await import('@/app/terms/page');
    const meta = await generateMetadata();
    expect(meta.alternates?.canonical).toBe('https://custom-domain.com/terms');
  });
  ...
  it('title is static (does not include site name)', async () => {
    ...
    const meta = await generateMetadata();
    expect(meta.title).toBe('Terms of Service');
    // Site name is added by layout title template
  });

  it('description mentions Holibob Platform Network', async () => {
    const { generateMetadata } = await import('@/app/terms/page');
    const meta = await generateMetadata();
    expect(meta.description).toContain('Holibob Platform Network');
  });
});
