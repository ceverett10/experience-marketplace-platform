...
export function LiveActivityIndicator({ count = 0, className = '' }: { count?: number; className?: string }) {
  ...
  return (
    <div className={`flex items-center gap-2 text-sm ${className}`}>
      ...
      <span className="text-gray-600">
        <strong className="font-medium text-gray-900">{count}</strong> {count === 1 ? 'person is' : 'people are'} viewing this
      </span>
    </div>
  );
}
...
export function PopularityBadge({
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 rounded-full bg-rose-100 px-3 py-1 text-sm ${className}`}>
