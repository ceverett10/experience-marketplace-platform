  ...
  describe('generateMetadata', () => {
    ...
    it('sets openGraph title with site name', async () => {
      ...
      expect(metadata.openGraph).toMatchObject({
        title: 'Terms of Service | Test Site',
        type: 'website',
      });
    });
    ...
    it('sets canonical URL using primaryDomain', async () => {
      const metadata = await generateMetadata();
      expect(metadata.alternates?.canonical).toBe('https://test.example.com/terms');
    });

    it('falls back to hostname when primaryDomain is not set', async () => {
